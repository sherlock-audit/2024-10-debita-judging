...
# Conflicting variable names render changeOwner function in AuctionFactory.sol ineffective
...
In AuctionFactory.sol::changeOwner() function, there is a name conflict. Indeed, the local variable `owner` (the function parameter) hides the state variable `owner` declared at the top of the contract.
